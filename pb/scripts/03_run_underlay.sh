...
if is_enabled "$svc"; then
  ...
  run_container "${CALDERA_CONTROLLER}" \
    ...
    -e "TELEMETRY_BASE_URL=http://${PROXY}:8080" \
    -e "TELEMETRY_EXPORT_ENABLED=${TELEMETRY_EXPORT_ENABLED:-true}" \
    -e "TELEMETRY_EXPORT_INTERVAL_SEC=${TELEMETRY_EXPORT_INTERVAL_SEC:-60}" \
    -e "TELEMETRY_OUTPUT_ROOT=${TELEMETRY_OUTPUT_ROOT:-/results/telemetry}" \
    -e "TELEMETRY_LOGS_LIMIT=${TELEMETRY_LOGS_LIMIT:-0}" \
    -e "TELEMETRY_LOGS_BATCH_SIZE=${TELEMETRY_LOGS_BATCH_SIZE:-5000}" \
    -e "TELEMETRY_LOGS_SCROLL_TTL=${TELEMETRY_LOGS_SCROLL_TTL:-1m}" \
    -e "TELEMETRY_TRACE_LIMIT=${TELEMETRY_TRACE_LIMIT:-5000}" \
    -e "TELEMETRY_METRICS_QUERY=${TELEMETRY_METRICS_QUERY:-{__name__!=\"\"}}" \
    -e "TELEMETRY_METRICS_STEP=${TELEMETRY_METRICS_STEP:-15s}" \
    -e "TELEMETRY_OPENSEARCH_USER=${TELEMETRY_OPENSEARCH_USER:-}" \
    -e "TELEMETRY_OPENSEARCH_PASS=${TELEMETRY_OPENSEARCH_PASS:-}" \
    -e "ADV_LIST=${ADV_LIST}" \
