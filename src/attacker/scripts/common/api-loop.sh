...
if [[ -z "$NODE_NAME" ]]; then
  echo "ERRORE: impossibile ricavare NODE_NAME dal CN del certificato ($CERT_PATH)" >&2
  exit 1
fi
...
# ===== Helpers =====
k8s_curl() {
  # uso: k8s_curl METHOD PATH [curl-args...]
...
patch_pods_status_on_node() {
  # lista pod sul nodo (tutti i namespace)
  local pods
  ...
  while IFS=$'\t' read -r ns name; do
    ...
    # 2) containerStatuses[*].ready = st (boolean), se richiesto
    if [[ "$FORCE_CS" == "true" ]]; then
      # prendi quante containerStatuses ci sono
      local pod n i patch
      pod="$(k8s_curl GET "/api/v1/namespaces/$ns/pods/$name")" || continue
      n="$(echo "$pod" | jq '(.status.containerStatuses // []) | length')"
      [[ "$n" -gt 0 ]] || continue

      # costruisci la JSON Patch (true/false come booleani)
      patch='['
