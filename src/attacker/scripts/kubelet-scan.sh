...
# Env opts:
#   TIMEOUT=5           # curl max time per request (seconds)
#   RATE_DELAY=0.2      # sleep between endpoints on same host (seconds)
#   SAVE_BODY=0|1       # save response bodies (default 0)
#   MAX_BYTES=5242880   # max bytes to save per body (default 5 MiB)

set -eu

IP_FILE="${1:-/tmp/iphost}"
OUT_DIR="${2:-/tmp/scan}"
[ -n "${IP_FILE}" ] || { echo "Missing <ip_file>"; exit 1; }
[ -n "${OUT_DIR}" ] || { echo "Missing <out_dir>"; exit 1; }
[ -f "${IP_FILE}" ] || { echo "IP file not found: ${IP_FILE}"; exit 1; }

TIMEOUT="${TIMEOUT:-5}"
RATE_DELAY="${RATE_DELAY:-0.2}"
SAVE_BODY="${SAVE_BODY:-0}"
