...
const checkoutLogsBufferSize = 256
...
func NewOTLPLogHook(ctx context.Context, serviceName string, scopeName string) (*OTLPLogHook, error) {
	...
	hook := &OTLPLogHook{
		...
		scopeName: scopeName,
		records:   make(chan *logspb.LogRecord, checkoutLogsBufferSize),
	}
...
func (h *OTLPLogHook) Fire(entry *logrus.Entry) error {
	...
	default:
		fmt.Fprintln(os.Stderr, "[checkout] dropping OTLP log record because buffer is full")
	}
...
func (h *OTLPLogHook) run() {
	...
	for record := range h.records {
		...
		cancel()
		if err != nil {
			fmt.Fprintf(os.Stderr, "[checkout] OTLP logs export failed: %v\n", err)
		}
	}
}
