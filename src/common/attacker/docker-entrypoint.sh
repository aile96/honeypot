...
ATTACKER_START_SCRIPT="${ATTACKER_START_SCRIPT-/opt/caldera/start.sh}"
...
if [[ "${DOCKER_DAEMON:-0}" == "1" ]]; then
  ...
  # - the local registry endpoint is derived from REGISTRY_NAME and REGISTRY_PORT.
  log "Starting dockerd (data-root=$DOCKER_DIR, host=$DOCKER_HOST_UNIX, driver=vfs)..."
  dockerd \
    --data-root="$DOCKER_DIR" \
    --host="$DOCKER_HOST_UNIX" \
    --storage-driver=vfs \
    --insecure-registry "${REGISTRY_NAME:-registry}:${REGISTRY_PORT:-5000}" \
    >>"$DOCKER_LOG" 2>&1 &
