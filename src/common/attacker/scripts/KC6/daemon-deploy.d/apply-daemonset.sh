...
if ! command -v yq >/dev/null 2>&1; then
  echo "[KC6-605] installing yq dependency"
  DEBIAN_FRONTEND=noninteractive apt-get update -qq >/dev/null
  DEBIAN_FRONTEND=noninteractive apt-get install -y -qq yq >/dev/null
fi
...
wait_daemonset_ready() {
  local timeout="${DAEMONSET_WAIT_TIMEOUT_SECONDS:-180}"
  local deadline=$((SECONDS + timeout))
  local ds_json desired ready observed generation status_line

  echo "[*] Waiting for DaemonSet ${NAMESPACE}/${DS_NAME} to be ready..."
  while (( SECONDS < deadline )); do
    ds_json="$(curl -sS "${APISERVER}/apis/apps/v1/namespaces/${NAMESPACE}/daemonsets/${DS_NAME}" \
      "${AUTH[@]}" "${CURL_TLS[@]}")"
    desired="$(echo "$ds_json" | jq -r '.status.desiredNumberScheduled // 0')"
    ready="$(echo "$ds_json" | jq -r '.status.numberReady // 0')"
    observed="$(echo "$ds_json" | jq -r '.status.observedGeneration // 0')"
    generation="$(echo "$ds_json" | jq -r '.metadata.generation // 0')"
    status_line="observed=${observed}/${generation} ready=${ready}/${desired}"

    if [[ "$desired" -gt 0 && "$observed" -ge "$generation" && "$ready" -ge "$desired" ]]; then
      echo "[+] DaemonSet ready: ${status_line}"
      return 0
    fi
  ...
  echo "Timed out waiting for DaemonSet ${NAMESPACE}/${DS_NAME} readiness" >&2
  return 1
}
...
echo "$PODS" | jq -e '.items | length > 0 and all(.[]; .status.phase == "Running")' >/dev/null
