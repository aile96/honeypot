...
TARGET_TAG="${TARGET_TAG:-${FRONTEND_PROXY_IMAGE_TAG:-${IMAGE_VERSION:-2.0.2}}}"
...
registry_prepare_build_args() {
  ...
  IMAGE_REF="${REGISTRY_ENDPOINT}/${TARGET_IMAGE}:${TARGET_TAG}"
  ...
  for var_name in "${TEMPLATE_VARS[@]}"; do
    value=""
    case "${var_name}" in
      CALDERA_HOST) value="${CALDERA_HOST_FROM_ADDR}" ;;
      CALDERA_PORT) value="${CALDERA_PORT}" ;;
...
registry_build_and_push() {
  ...
  printf '%s' "${REGISTRY_PASS_FROM_FILE}" | docker login "${REGISTRY_ENDPOINT}" -u "${REGISTRY_USER_FROM_FILE}" --password-stdin
  docker build "${BUILD_ARGS[@]}" -f "${PROXY_DOCKERFILE}" -t "${IMAGE_REF}" "${PROXY_CONTEXT}"
  docker push "${IMAGE_REF}"
  if ! docker manifest inspect "${IMAGE_REF}" >/dev/null 2>&1; then
    warn "docker manifest inspect failed for ${IMAGE_REF}; push completed, continuing."
  fi
}
