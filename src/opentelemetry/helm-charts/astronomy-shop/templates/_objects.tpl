...
{{- $defaultTag := .defaultValues.image.tag | default .Chart.AppVersion }}
---
apiVersion: apps/v1
kind: Deployment
metadata:
...
spec:
  ...
  template:
    ...
    spec:
      ...
      containers:
        - name: {{ .name }}
          {{- $imageRepository := tpl (((.imageOverride).repository) | default (printf "%s/%s" .defaultValues.image.repository .name)) . }}
          image: '{{ $imageRepository }}:{{ ((.imageOverride).tag) | default $defaultTag }}'
        ...
        - name: {{ .name   }}
          {{- $sidecarImageRepository := tpl (((.imageOverride).repository) | default (printf "%s/%s" .defaultValues.image.repository .name)) . }}
          image: '{{ $sidecarImageRepository }}:{{ ((.imageOverride).tag) | default $defaultTag }}'
          imagePullPolicy: {{ ((.imageOverride).pullPolicy) | default .defaultValues.image.pullPolicy }}
          {{- if .command }}
          command:
            {{- .command | toYaml | nindent 12 -}}
