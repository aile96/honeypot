...
reachability, records smoke-test results, and stops the temporary build-helper
when Skaffold used one."""
...
def run_smoke_tests() -> None:
    ...
    caldera_port = config_str(CONFIG, "CALDERA_PORT", 8888)
    checks["caldera_http"] = wait_http(f"http://127.0.0.1:{caldera_port}", timeout_seconds=10)
    ...
    if not checks["caldera_http"]:
        warn("Caldera is not reachable at http://127.0.0.1:8888; kill-chain runner will report this clearly.")
...
def stop_skaffold_build_helper_if_started() -> None:
    """Remove the Skaffold build-helper started by HOOK_PRE_03, if any."""
    if not get_state_value(STATE, "skaffold_build_helper_started", False):
        return
    ...
    try:
        ...
        warn(f"Could not stop cluster build helper: {exc}")
        set_state_value(STATE, "skaffold_build_helper_stopped", False)
